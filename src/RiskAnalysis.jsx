...
const AI_API = "/api/ai";
const authHeaders = () => { const token=localStorage.getItem("token")||localStorage.getItem("fraudshield-token")||""; return token ? {Authorization:`Bearer ${token}`} : {}; };
...
export default function RiskAnalysis({
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          window.dispatchEvent(new CustomEvent("fraudshield:logout"));
      ...
      if (!data) {
        setStatus("error");
        setError(
          "Hugging Face analysis is unavailable."
        );
        return;
      }
      ...
      setStatus("error");
      setError(
        "The AI risk-analysis service is unavailable. Make sure the Node and Hugging Face AI services are running."
      );
    }
  }
  ...
  const level =
    status === "done" && result
      ? LEVEL_STYLES[result.risk_level] || NOT_ANALYZED_STYLE
      : NOT_ANALYZED_STYLE;

  const score =
    status === "done" && result
      ? Math.min(Math.max(Number(result.risk_score) || Number(result.score) || 0, 0), 100)
      : 0;
        ...
        <div className="content">
          ...
          <div className="panel">
            ...
            >
              {status === "done" && result
                ? result.explanation ||
                  "Hugging Face scam analysis completed."
                : status === "loading"
                  ? "Running Hugging Face scam analysis..."
                  : status === "error"
                    ? error
                    : "Enter text above and click Analyze Text to run the Hugging Face risk check."}
            </p>
          </div>
            ...
            >
              ...
              >
                ...
                result?.explanation
                  ? result.explanation
                  : status === "error"
                    ? "No explanation available. The Hugging Face risk-analysis service could not be reached."
